...
function QTAssessment() {
  const steps = [
    ...
    {
      title:
        "Step 1: Use Fredericia's formula (QTcF) for correction (Cardiology tab in Results View)",
      subPoints: [],
      nextStep: 2,
    },
    {
    ...
    },
    {
      ...
      subPoints: [
        {
          text: "Evaluate for reversible causes:",
          options: [
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      setAnimate(true);
    }, 10); // Small delay to ensure the new content is rendered before animating
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      ...
      if (step >= 0 && step < steps.length) {
        setCurrentStep(step);
        setAnimate(false); // Reset animation when navigating via history
      }
    };
  ...
  const handleOptionClick = (optionText) => {
    ...
    if (
      ...
      setAnimate(false); // Reset animation for the new step
    } else {
            ...
            <div className="step-column">
              ...
              {currentStepData.subPoints.map((point, pointIndex) => (
                <div key={pointIndex} className="sub-point">
                  {typeof point === "string" ? (
                    <p
                      className="hyphen-point"
                      onClick={() => handleOptionClick(point)}
                        ...
                        <ul className="bullet-list">
                          {point.options.map((option, optIndex) => {
                            const isProceedOption =
                              ...
                              (option.includes("Proceed to Step") ||
                                option.includes("then Step"));
                            return (
                              <li
                                key={optIndex}
                                onClick={
                                ...
                                style={{
                                  cursor: isProceedOption
                                    ? "pointer"
                                    : "default",
                                  color: isProceedOption
                                    ? "#646cff"
                                    : "inherit",
                                }}
                              >
